...
class Encoder(tf.keras.layers.Layer):
    """Model to embed genotypes to a latent dimension.

    3D input will be flattened to (n_samples, n_features * num_classes). Model output is a reduced-dimensional representation (i.e., embedding) of the input of size ``latent_dim``\.

    Args:
        n_features (int): Number of features to input into model.
        num_classes (int): Number of classes in input.
        latent_dim (int): Number of latent dimensions to output (i.e., n_components).
        hidden_layer_sizes (int, str, List[int], or List[str]): Scalar or list of hidden layer sizes to use in hidden layers. Possible string options include {"auto", "sqrt", "log2"}. If a list is passed, it must be the same length as num_hidden_layers.
        dropout_rate (float): Dropout rate for neural network.
        activation (str): Hidden activation function to use.
        kernel_initializer (str): Function to use to initialize weights. See tf.keras.initializers for more info.
        kernel_regularizer (str): Regularizer function to use with neural network.
    ...
    def call(self, inputs, training=None):
        """Forward pass for neural network model.

        Args:
            inputs (tf.Tensor): Input tensor. Must be one-hot encoded.
            training (bool): Whether doing training or not. Defaults to None.
...
class Decoder(tf.keras.layers.Layer):
    """Converts the embedded vector back into the reconstructed output.
...
class AutoEncoderModel(tf.keras.Model):
    ...
    Args:
        y (numpy.ndarray): Input data of shape (n_samples, n_features, num_classes).
    ...
    @property
    def metrics(self):
        """Define metrics to keep track of during training."""
        return [
    ...
    def train_step(self, data):
        """Custom train step per batch during fit."""
        y = self._y
    ...
    def test_step(self, data):
        """Custom test step per batch during fit."""
        y = self._y
    ...
    def missing_mask(self):
        """Missing mask, with missing values set to True."""
    ...
    def sample_weight(self, value):
        """Set sample weights after each epoch."""
