...
try:
    ...
    from .callbacks import (
        UBPCallbacks,
        AutoEncoderCallbacks,
    )
    from .keras_classifiers import MLPClassifier, AutoEncoderClassifier
...
class SAE(BaseEstimator, TransformerMixin):
    """Class to impute missing data using an Autoencoder (SAE) neural network.
    ...
    @timer
    def fit(self, X):
        """Train the autoencoder model on input data X.
    ...
    def run_clf(
        ...
        **kwargs,
        ...
        if self.run_gridsearch_:
            ...
            fp = os.path.join(
                f"{self.prefix}_output", "reports", "autoencoder_cvresults.csv"
            )
    ...
    def _initialize_parameters(self, y_train):
        ...
        compile_params = self.nn_.set_compile_params(self.optimizer, ae=True)
...
class UBP(BaseEstimator, TransformerMixin):
    ...
    def _create_model(
        ...
        Args:
            ...
            n_components (int): The number of principal components to use with NLPCA or UBP models. Not used if doing autoencoder. Defaults to 3.
